...
A `str_view()` é uma função apenas didática que serve para visualizar um padrão em uma *string*. Aqui procuramos pelo padrão "*am*" e a função destaca este padrão nos elementos (se encontrado) com os símbolos `<>`.
...
Aqui é explorado as **operações básicas do pandas** para trabalhar com tabelas:

- Colunas:
  - **Renomear colunas** &rarr; *isso é o que veremos hoje!*
  - Selecionar colunas
...
Estas são operações do dia a dia de uma análise de dados e veremos como utilizar **a biblioteca `pandas`** para facilitar o trabalho.

Grande parte dos procedimentos que precisamos fazer para **transformar dados brutos em informação útil** podem ser feitos com funções da biblioteca `pandas`, através dos objetos `Series` ou `DataFrame`.

Algumas **funções que iremos explorar** adiante, por componente de uma tabela sobre o qual o verbo se aplica:

- Colunas:
  - **`rename()` muda o nome das colunas** &rarr; *isso é o que veremos hoje!*
...
Uma dica bastante útil para quando você tiver nomes de colunas mal formatados ou pouco convencionais: use a biblioteca `pyjanitor` para converter e limpar, com algumas convenções e boas práticas, todos os nomes de colunas automaticamente:
...
Para filtrar determinadas linhas de uma tabela usamos a função `query()` com a sintaxe `tabela.query("condicao")`, onde `condicao` é uma expressão lógica dentro de uma *string*, na linguagem interna do `pandas`, aplicada aos valores de uma coluna. As linhas que serão mantidas após o `query()` são as linhas onde `condicao` é verdadeiro (`True`). Para exemplificar, suponha que você queira todas as linhas de uma tabela que possui o valor `2023` na coluna `DataReferencia`, para tal filtro use uma condição de igualdade:
...
Ao usar groupby e agg() em conjunto, as operações que poderiam parecer complicadas se tornam bastante simples. Por exemplo, se você quiser obter a média mensal de respondentes no Focus, basta definir o(s) grupo(s), acessar a coluna sobre a qual deve ser aplicada a operação e apontar a função em agg():
